...
class AllocEventGenerator : public Generator<AllocEvent> {
    private:
        ...
        AllocEvent step_lcg() {
            state = state * 6364136223846793005ULL + 1442695040888963407ULL; // Формула генератора псевдослучайных чисел LCG, 2 константы из LCG Кнута
            uint64_t r1 = state >> 33; // Берем старшие биты состояния
    ...
    public:
        AllocEventGenerator(uint64_t seed, int existing_blocks_cap, int max_alloc_size, int alloc_prob_percent) : initial_seed(seed),
              state(seed), existing_blocks_cap(existing_blocks_cap), max_alloc_size(max_alloc_size),
              alloc_prob_percent(alloc_prob_percent), pos(0) {}
...
// existing_blocks_cap - диапазон id для Free-событий от 0 до existing_blocks_cap
// max_alloc_size - Alloc даст size из от 1 до max_alloc_size
// alloc_prob_percent - доля Alloc от 0 до 100
inline LazySequence<AllocEvent>* make_alloc_event_stream(uint64_t seed,  int existing_blocks_cap = 32, int max_alloc_size = 5, int alloc_prob_percent = 70) {
